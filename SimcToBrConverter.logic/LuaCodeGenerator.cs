 ...
 public class LuaCodeGenerator
    {
        ...
        internal static void GenerateActionLineLuaCode(ConversionResult conversionResult)
        {
            ...
            List<ILuaGenerator> luaGenerators = ProfileProcessor.LuaGenerators;
        ...
        public static string GenerateLuaCode()
        {
            // Generate Action Lists
            string lastListName = "";
            StringBuilder processedActionListLines = new();
            foreach (var conversionResult in ProfileProcessor.ConversionResults.Where(a => !string.IsNullOrEmpty(a.ActionLine.ListName)))
            {
                ...
                ProfileProcessor.PreviousActionLine = conversionResult.ActionLine;
            }

            // Generate Locals
            StringBuilder processedLocals = new();
            ProfileProcessor.Locals.Add("actionList"); // Add actionList to the list of locals (it's always used)
            ProfileProcessor.Locals.Add("ui"); // Add ui to the list of locals (it's always used)

            var locals = ProfileProcessor.Locals
                .Select(s => s.Replace("()", ""))
                .Distinct()
                .OrderBy(s => s)
                .ToList();
            ...
            foreach (var extraVar in ProfileProcessor.ExtraVars)
            {
