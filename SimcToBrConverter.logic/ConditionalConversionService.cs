 ...
 public class ConditionConversionService
    {
        ...
        private static void AddToLocalList(string local)
        {
            ...
            {
                ProfileProcessor.Locals.Add(local);
            }
        }

        public void ConvertCondition()
        {
            ActionLine actionLine = ProfileProcessor.CurrentActionLine;
            List<string> notConvertedConditions = ProfileProcessor.NotConverted;
            ...
            ProfileProcessor.CurrentActionLine = actionLine;
            ProfileProcessor.NotConverted = notConvertedConditions;
        }
        ...
        private string GatherSubExpression(List<string> conditions, int startIndex, bool reverse)
        {
            ...
            while (index >= 0 && index < conditions.Count)
            {
                ...
                {
                    part = ConvertConditionPart(part, ProfileProcessor.CurrentActionLine.Action, ref ProfileProcessor.NotConverted);
                }
        ...
        {
            var tempActionLine = ProfileProcessor.CurrentActionLine;
            ProfileProcessor.CurrentActionLine = actionLine;
            ProfileProcessor.NotConverted = new List<string>();
            ConvertCondition();
            var convertedActionLine = ProfileProcessor.CurrentActionLine;
            var notConvertedConditions = ProfileProcessor.NotConverted;
            ProfileProcessor.CurrentActionLine = tempActionLine;
